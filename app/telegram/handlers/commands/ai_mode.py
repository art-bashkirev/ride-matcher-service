...
async def function(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not db_user:
            logger.warning(
                "User %s attempted to use ai_mode without starting bot first", user_info
            )
        ...
        if not db_user.is_admin:
            logger.warning(
                "Non-admin user %s attempted to use ai_mode command", user_info
            )
            await update.message.reply_text(get_message("ai_mode_admin_only"))
            return
    except Exception as e:
        logger.error(
            "Database error while checking user %s for ai_mode command: %s",
            user_info,
            e,
        )
    ...
    if not args:
        ...
        try:
            ...
            status_word = get_message(
                "ai_mode_status_enabled"
                if current_status
                else "ai_mode_status_disabled"
            )
    ...
    if command not in ["on", "off", "enable", "disable", "true", "false"]:
        ...
        await update.message.reply_text(
            get_message("ai_mode_invalid_command"), parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        if success:
            status_word = get_message(
                "ai_mode_status_enabled" if enable_ai else "ai_mode_status_disabled"
            )
            await update.message.reply_text(
            ...
            logger.info(
                "Admin user %s successfully changed AI mode to: %s",
                user_info,
                enable_ai,
            )
        else:
            ...
            logger.error(
                "Failed to update AI mode for admin user %s (operation returned false)",
                user_info,
            )
    except Exception as e:
        logger.error(
            "Exception while updating AI mode for admin user %s: %s", user_info, e
        )
