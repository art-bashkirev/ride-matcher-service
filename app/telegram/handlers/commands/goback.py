...
async def goback_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if telegram_id is None or update.message is None:
        logger.warning("goback_command called with no user or message")
        return
    
    # Log command invocation
    logger.info("User %s (username: %s) invoked /goback command", 
    ...
    if not db_user or not db_user.base_station_code or not db_user.destination_code:
        logger.warning("User %s attempted /goback without stations configured", telegram_id)
    ...
    try:
        ...
        # Create search request (REVERSED: destination -> base)
        search_req = SearchRequest(
            from_=db_user.destination_code,
            to=db_user.base_station_code,
            date=now.strftime('%Y-%m-%d'),
            result_timezone=config.result_timezone,
            limit=300  # Get all trains
        )
        
        logger.info("Fetching search results for user %s (reverse): %s -> %s", 
                   telegram_id, db_user.destination_code, db_user.base_station_code)
        ...
        logger.debug("Search results for user %s (reverse): cached=%s, segments=%d", 
                    telegram_id, was_cached, len(search_response.segments or []))
        
        if not search_response.segments:
            logger.warning("No train segments found for user %s (reverse)", telegram_id)
            ...
            try:
                ...
                if now <= departure_dt <= end_time:
                    ...
                    thread = CandidateThread(
                        ...
                        from_station_code=db_user.destination_code,
                        to_station_code=db_user.base_station_code,
                        from_station_title=db_user.destination_title or "",
                        to_station_title=db_user.base_station_title or ""
                    )
        ...
        logger.info("User %s has %d candidate trains in time window (reverse)", 
        ...
        if not success:
            logger.error("Failed to store search results for user %s (reverse)", telegram_id)
        ...
        logger.info("User %s has %d matching threads with other users (reverse)", 
        ...
        logger.info("User %s /goback command completed successfully", telegram_id)
        
    except Exception as e:
        logger.error("Error in goback_command for user %s: %s", telegram_id, e, exc_info=True)
        await searching_msg.edit_text(get_message("ride_search_error"))
