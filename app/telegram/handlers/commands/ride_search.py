...
async def search_rides(
    update: Update, context: ContextTypes.DEFAULT_TYPE, reverse: bool = False
):
    """
    ...
    command = "goback" if reverse else "goto"
    logger.info(
        "User %s (username: %s) invoked /%s command",
        telegram_id,
    ...
    if not db_user or not db_user.base_station_code or not db_user.destination_code:
        logger.warning(
            "User %s attempted /%s without stations configured", telegram_id, command
        )
    ...
    searching_msg = await update.message.reply_text(
        get_message("ride_search_searching")
    )
    ...
    try:
        ...
        logger.info(
            "Fetching search results for user %s%s: %s -> %s",
            telegram_id,
        ...
        logger.debug(
            "Search results for user %s%s: cached=%s, segments=%d",
            telegram_id,
            " (reverse)" if reverse else "",
            was_cached,
        ...
        if not search_response.segments:
            logger.warning(
                "No train segments found for user %s%s",
                telegram_id,
                " (reverse)" if reverse else "",
            )
        ...
        logger.info(
            "User %s has %d candidate trains in time window%s",
            telegram_id,
        ...
        if not success:
            logger.error(
                "Failed to store search results for user %s%s",
                telegram_id,
                " (reverse)" if reverse else "",
            )
        ...
        if users_to_notify:
            ...
            for user_info in users_to_notify:
                try:
                    ...
                    logger.debug(
                        "Notified user %s about new match", user_info["telegram_id"]
                    )
                except Exception as e:
                    logger.error(
                        "Failed to notify user %s: %s", user_info.get("telegram_id"), e
                    )
        ...
        logger.info(
            "User %s has %d matching threads with other users%s",
            telegram_id,
            len(matches),
            " (reverse)" if reverse else "",
        )
        ...
        if matches:
            ...
            for thread_uid, matched_users in matches.items():
                ...
                thread_info = next(
                    (t for t in candidate_threads if t.thread_uid == thread_uid), None
                )
