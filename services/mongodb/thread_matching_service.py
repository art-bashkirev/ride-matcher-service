...
class ThreadMatchingService:
    ...
    async def _get_client(self) -> AsyncMongoClient:
        ...
        if self._client is None:
            ...
            logger.info(
                "MongoDB client created for thread matching service using %s",
    ...
    async def _get_db(self) -> AsyncDatabase:
        ...
        if self._db is None:
            ...
            db_name = self.config.mongodb_database or "ride_matcher"
            self._db = cast(AsyncDatabase, client[db_name])
    ...
    async def get_search_results_collection(self):
        ...
        collection_name = (
            self.config.mongodb_search_results_collection or "user_search_results"
        )
        collection = cast(AsyncCollection, db[collection_name])

        # Create TTL index on expires_at field (if not exists)
        try:
            await collection.create_index("expires_at", expireAfterSeconds=0)
            await collection.create_index("telegram_id")
            await collection.create_index(THREAD_UID_FIELD)
            logger.debug(
                "TTL and query indexes ensured on %s collection", collection_name
            )
        except Exception as e:
            logger.warning("Failed to create indexes on %s: %s", collection_name, e)
    ...
    async def store_search_results(
        ...
        try:
            ...
            await collection.replace_one(
                {"telegram_id": telegram_id}, search_results.model_dump(), upsert=True
            )
            ...
            logger.error(
                "Failed to store search results for user %s: %s", telegram_id, e
            )
            return False
    ...
    async def find_matches(self, telegram_id: int) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            user_threads = {
                ct["thread_uid"] for ct in user_doc.get("candidate_threads", [])
            }
            if not user_threads:
            ...
            logger.info(
                "Finding matches for user %s with %d candidate threads",
                telegram_id,
                len(user_threads),
            )
            ...
            cursor = collection.find(
                {
                    THREAD_UID_FIELD: {"$in": list(user_threads)},
            ...
            logger.info(
                "Found %d matching threads for user %s", len(result), telegram_id
            )
            return result
    ...
    async def find_users_to_notify(
        self, new_user_telegram_id: int, new_user_threads: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            new_user_doc = await collection.find_one(
                {"telegram_id": new_user_telegram_id}
            )
            if not new_user_doc:
                logger.warning(
                    "No document found for new user %s", new_user_telegram_id
                )
                return []
            ...
            cursor = collection.find(
                {
                    THREAD_UID_FIELD: {"$in": new_user_threads},
                    "telegram_id": {
                        "$ne": new_user_telegram_id
                    },  # Exclude the new user
                }
            )
            ...
            async for other_doc in cursor:
                ...
                if matching_thread_uids:
                    ...
                    user_info = {
                        ...
                        "new_user_name": (
                            new_user_doc.get("first_name")
                            or new_user_doc.get("username")
                            or "Пользователь"
                        ),
    ...
    async def clear_search_results(self, telegram_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to clear search results for user %s: %s", telegram_id, e
            )
            return False
